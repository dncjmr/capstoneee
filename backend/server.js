...
app.put("/api/queue/:queueNumber", async (req, res) => {
  try {
    ...
    // If marked as seated, notify via Socket
    if (status === "Seated") {
    ...
    // Return the updated queue row
    const [updated] = await db.query("SELECT * FROM queue WHERE queue_number = ?", [queueNumber]);
    res.json({ success: true, queue: updated[0] });
  } catch (err) {
...
app.get("/api/admin/queue/overview", authMiddleware(["admin"]), async (req, res) => {
  try {
    ...
    const total_today = rows.filter(q => q.created_at.startsWith(todayStr)).length;
    const waiting_today = rows.filter(q => q.created_at.startsWith(todayStr) && q.status === "waiting").length;
    const seated_today = rows.filter(q => q.created_at.startsWith(todayStr) && q.status === "Seated").length;
    const total_all = rows.length;
    const seated_all = rows.filter(q => q.status === "Seated").length;

    res.json({ total_today, total_all, waiting_today, seated_today, seated_all });
  } catch (err) {
...
app.get("/api/admin/orders/total-revenue", authMiddleware(["admin"]), async (req, res) => {
  try {
    const [rows] = await db.query(
      "SELECT COALESCE(SUM(total_price),0) AS totalRevenue FROM orders WHERE status='Served'"
    );
